...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <section className="workspace" id="top">
      ...
      {mode === "file" ? <button className="dropzone" onDrop={handleDrop} onDragOver={(event) => event.preventDefault()} onClick={() => inputRef.current?.click()} disabled={isBusy}><input ref={inputRef} type="file" accept=".pdf,.docx,.xls,.xlsx,.csv,.pptx,.png,.jpg,.jpeg,.bmp,.tiff,.webp" onChange={handleFile} hidden /><span className="upload-mark">↓</span><strong>{isBusy ? "Converting your file…" : selectedFile ? selectedFile.name : "Drop a file here"}</strong><span>{selectedFile ? "ready to convert and download" : "or click to browse"}</span><small>PDF · DOCX · XLS/XLSX · CSV · PPTX · images</small></button> : <div className="url-box"><label htmlFor="url">Web page address</label><div><input id="url" type="url" placeholder="https://example.com/article" value={url} onChange={(event) => setUrl(event.target.value)} /><button onClick={requestDownload} disabled={isBusy || !url.trim()}>Download <span>↓</span></button></div><p>It will be converted only when you download it.</p></div>}
